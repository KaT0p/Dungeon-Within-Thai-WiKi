...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const description = translated ? `คำแปลไทย–อังกฤษของสกิลและชิ้นส่วน ${translated.name} Tier ${translated.tier}` : '';
  ...
  if (!monster) {
    ...
    return <main className="hub-page"><SiteHeader />
      <section className="monster-detail-hero rose">
        <div><p className="breadcrumbs"><a href="/wiki">คลังข้อมูล</a><span>/</span><a href="/wiki/monsters">มอนสเตอร์</a><span>/</span>{translated.name}</p><span className="status-pill review">ชื่อแปลแล้ว · เอฟเฟกต์รอตรวจ</span><h1>{translated.name}</h1><p>รวมชื่อภาษาไทยและอังกฤษของการโจมตีพื้นฐาน คุณลักษณะติดตัว และชิ้นส่วนทั้งสามช่อง โดยคงชื่ออังกฤษไว้สำหรับค้นในเกม</p><div className="monster-hero-tags"><b>Tier {tier}</b><span>{type}</span><span>สกิล 5 รายการ</span></div></div>
      ...
      <section className="detail-layout"><article className="detail-main">
        ...
        <section><div className="section-title-row"><h2>สกิลและชิ้นส่วน</h2><span>คำแปลไทย + ชื่ออังกฤษ</span></div><div className="monster-skill-list">{translated.skills.map((skill) => <article key={skill.slot}><div className="monster-skill-head"><b>{skill.slot}</b><div><h3>{skill.th}</h3><small>{skill.en}</small></div><span>{skill.kind}</span></div><p>แปลชื่อแล้ว รายละเอียดเอฟเฟกต์ ตัวเลข สูตร และเงื่อนไขการทอยยังต้องตรวจเทียบกับ Codex ในเกม v0.6.0</p></article>)}</div></section>
      ...
      </article><aside className="detail-side"><div><small>Wiki ต้นทางแก้ไขล่าสุด</small><b>{translated.sourceUpdated}</b></div><div><small>สถานะ</small><b>ชื่อแปลแล้ว · รอตรวจเอฟเฟกต์</b></div><a href="/wiki/monster-skills">เปิดคลังคำแปลทั้งหมด →</a><a href={`https://dungeonwithin.miraheze.org/wiki/${translated.name.replaceAll(' ', '_')}`} target="_blank" rel="noreferrer">เปิด Wiki ต้นทาง ↗</a><a href="/wiki/monsters">← กลับบัญชีมอนสเตอร์</a></aside></section><SiteFooter />
    </main>;
  }
