...
export default function MonsterSkillsPage() {
  ...
  return <main className="hub-page"><SiteHeader />
    <section className="data-hero monster-hero"><p className="breadcrumbs"><Link href="/wiki">คลังข้อมูล</Link><span>/</span><Link href="/wiki/monsters">มอนสเตอร์</Link><span>/</span>คำแปลสกิล</p><p className="eyebrow"><span /> Tier 1–5 Skill Archive</p><h1>คำแปลสกิลมอนสเตอร์</h1><p>ค้นชื่อไทยหรืออังกฤษของการโจมตีพื้นฐาน คุณลักษณะติดตัว และชิ้นส่วนทั้ง 3 ช่อง สำหรับมอนสเตอร์ Tier 1–5</p></section>
    <section className="hub-content monster-skill-catalog">
      <div className="inventory-summary"><div><small>มอนสเตอร์</small><b>{monsterSkillTranslations.length}</b></div><div><small>ชื่อที่แปล</small><b>{monsterSkillCount}</b></div><div><small>ช่วง Tier</small><b>1–5</b></div><div><small>รอข้อมูลต้นทาง</small><b>{monsterSkillTranslations.filter((monster) => !monster.sourceAvailable).length}</b></div></div>
      <div className="monster-warning"><strong>สถานะคำแปล</strong><p>ชื่อไทยทั้งหมดเป็นคำแปลแฟนเมด ส่วนชื่ออังกฤษคงตาม Wiki ต้นทางเพื่อใช้ค้นในเกม Tier 4–5 มี 13 ตัวที่ Wiki ยังไม่มีหน้ารายละเอียด จึงแสดงสถานะรอข้อมูลโดยไม่แต่งชื่อสกิล ตัวเลข หรือเอฟเฟกต์ขึ้นเอง</p></div>
      <div className="monster-tools"><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหามอนสเตอร์ ชื่อไทย หรือชื่ออังกฤษ..." /></label><select aria-label="กรอง Tier" value={tier} onChange={(event) => setTier(event.target.value)}><option>ทั้งหมด</option>{[1, 2, 3, 4, 5].map((value) => <option key={value} value={value}>Tier {value}</option>)}</select><select aria-label="กรองช่องสกิล" value={slot} onChange={(event) => setSlot(event.target.value as (typeof slots)[number])}>{slots.map((value) => <option key={value} value={value}>{value === 'ทั้งหมด' ? value : `${value} · ${value === 'A' ? 'Attack' : value === 'B' ? 'Innate' : 'Part'}`}</option>)}</select></div>
