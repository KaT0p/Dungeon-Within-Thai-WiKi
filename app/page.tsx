...
const searchIndex = [
  ...
  { title: 'สกิลมอนสเตอร์ Tier 1–5', description: 'คำแปลไทย–อังกฤษ 160 ชื่อ พร้อมสถานะตัวที่ยังไม่มีข้อมูลต้นทาง', category: 'มอนสเตอร์', href: '/wiki/monster-skills', keywords: monsterSkillTranslations.map((monster)=>`${monster.name} ${monster.skills.map((skill)=>`${skill.en} ${skill.th}`).join(' ')}`).join(' ') },
...
export default function Home() {
  ...
  return <main><SiteHeader />
    ...
    <section className="guide-strip" id="guides"><div><p className="eyebrow"><span /> เพิ่มล่าสุด</p><h2>สกิลมอนสเตอร์ Tier 4–5</h2></div><p>เพิ่มคำแปล 30 ชื่อจาก 6 ตัวที่มีข้อมูลต้นทาง<br />และหน้าสถานะรอข้อมูลสำหรับอีก 13 ตัว</p><Link className="primary-button" href="/wiki/monster-skills">เปิดคลังสกิล <span>→</span></Link></section><SiteFooter />
  </main>;
}
